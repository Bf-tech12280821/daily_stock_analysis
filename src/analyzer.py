...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def _format_prompt(
        ...
        你当前执行的是**早盘轻量模式**，目标不是做完整复盘，而是帮助用户快速判断：
        **今天市场会不会因为新信息改变预期，这只股票今天值不值得看、等、跟，还是该避开。**

        请严格遵守以下要求：

        1. **优先分析消息面，而不是先做长篇技术复盘**
        - 请先分析**隔夜至近24小时内**与该股票直接相关的消息，优先判断今天是否出现了会影响短线预期的“新变量”。
        - 如果没有真正的新消息，要明确说明：当前消息更多是旧逻辑延续，而不是新增催化。
        - 不要把旧闻重复包装成强利好。
        ...
        - **影响周期**：短期扰动 / 中期催化 / 趋势变量
        - **归属类型**：行业消息 / 公司专属消息
        - **盘面确认条件**：该消息是否已经被盘面确认，或者今天需要通过什么表现来确认（如高开高走、放量站上关键位、承接增强等）

        3. **技术面只保留最关键的交易确认信息**
        不要展开长篇趋势复盘，不要完整重复 full 模式中的大段技术模板。

        技术面只保留以下内容：
        - **当前位置**：当前偏强 / 偏弱 / 震荡
        - **关键支撑位**：只给 1 个最重要的
        - **关键压力位**：只给 1 个最重要的
        - **开盘/早盘情绪确认**：量价、承接、是否有异常强弱信号

        技术面的任务不是复盘过去，而是回答：
        **这些消息，今天有没有条件被市场拿来交易？**

        4. **若消息面与技术面冲突，必须明确谁对今天更重要**
        如果出现：
        - 消息偏多，但技术偏弱
        - 消息一般，但盘面超预期转强
        ...
        5. **最终输出必须简洁，像早盘决策卡，不像收盘长复盘**
        请优先帮助用户在开盘前/开盘初期快速做判断。
        ...
        6. **若消息面不足以支持明确交易结论**
        请优先输出“观察”或“等确认”，不要为了给结论而强行乐观或悲观。

        7. **禁止事项**
        在早盘轻量模式中，禁止出现以下问题：
        - 不要写成长篇完整趋势复盘
        - 不要展开 full 模式中的完整作战计划（买点/次买点/目标位/仓位/风控全套）
        - 不要把消息面只当作背景材料罗列
        - 不要只说“有利好”或“有消息”，必须说明它对今天股价的实际意义
        - 不要忽略“这条消息是新催化还是旧闻重提”
