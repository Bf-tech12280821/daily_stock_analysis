...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def _format_prompt(
        ...
        你当前执行的是**早盘轻量模式**。  
        目标不是做完整复盘，而是帮助用户快速判断：

        **今天市场会不会因为新信息改变预期，这只股票今天值不值得看、等、跟，还是该避开。**

        在**早盘轻量模式**中，必须严格遵守以下规则：

        1. **必须严格按照以下四个模块输出，禁止新增其他主模块**
        输出时，必须严格按以下顺序输出，且只能输出这四个模块：
        ### 一句话判断
        ### 消息面速览
        ### 技术确认
        ### 今日应对
        四个模块必须按顺序完整输出，不得调换顺序，也不得合并模块。
        ...
        - 完整买点/次买点/目标位/仓位配置
        - 长篇趋势复盘
        若输出中出现“重要信息速览、最新动态、数据透视、作战计划、检查清单、完整持仓策略、完整风控策略、完整买点/次买点/目标位/仓位配置、长篇趋势复盘”等任一旧模块标题，视为输出不合格，必须重新改写为四个指定模块。

        如需表达原本会放在旧模块中的内容，必须归入上述四个模块中：
        - 消息相关内容，一律归入 **消息面速览**
        - 技术位置、强弱、支撑压力，一律归入 **技术确认**
        - 交易动作建议，一律归入 **今日应对**

        2. **消息面优先，必须先写消息面，再写技术确认**
        必须优先分析**隔夜至近24小时内**与该股票直接相关的消息，先判断今天是否出现会影响短线预期的“新变量”。

        如果没有真正的新消息，必须明确说明：
        - 当前消息更多是旧逻辑延续，不是新增催化
        - 不能把旧闻重复包装成强利好
        若隔夜与近24小时内缺乏足够新消息，也必须从现有相关消息中选取至少 2 条，按固定字段完成分类；不得因此退回到“舆情情绪 / 业绩预期 / 风险警报 / 利好催化 / 最新动态”等旧式概括写法。

        3. **消息面速览必须逐条列出，并且每条都必须包含固定字段**
        消息面速览模块至少列出 2 条消息，必须逐条编号，不得只写总结性段落。

        每条消息都必须完整包含以下字段，不可省略，也不得用一句总括判断代替逐条分类：
        - 消息内容：
        - 性质：偏多 / 中性 / 偏空
        - 新旧程度：新催化 / 旧闻重提
        - 影响周期：短期扰动 / 中期催化 / 趋势变量
        ...
        如果某项无法判断，也必须明确写“无法判断”，不能跳过。

        4. **技术确认模块必须非常简短，只保留交易确认信息**
        技术确认模块只能写以下四项：
        - 当前强弱：偏强 / 偏弱 / 震荡
        - 一个关键支撑位
        - 一个关键压力位
        ...
        技术确认模块总长度尽量控制在 4 行左右。

        技术确认的核心任务不是复盘过去，而是回答：
        **这些消息，今天有没有条件被市场拿来交易？**

        5. **若消息面与技术面冲突，必须明确今天短线更该参考哪一方**
        如果出现：
        - 消息偏多，但技术偏弱
        - 消息一般，但盘面超预期转强
        ...
        若消息面不足以支持明确交易结论，请优先输出“观察”或“等确认”，不要为了给结论而强行乐观或悲观。

        7. **早盘轻量模式的核心判断标准**
        分析时必须始终围绕下面四个问题：
        1) 今天有没有新的预期变化？
        2) 这些消息是新催化，还是旧逻辑重复？
        3) 这些消息会影响今天，还是只影响中长期？
        4) 盘面今天是否有条件确认这些消息？
        """
        else:
            mode_rules = """
